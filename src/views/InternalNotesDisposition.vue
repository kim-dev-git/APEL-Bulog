...
export default {
  ...
  computed: {
    ...
    document() {
      return this.$store.state.internalnotes.document
    },
    disposition() {
      return this.$store.state.internalnotes.disposition
    }
  },
  methods: {
    get() {
      this.$store.dispatch('internalnotes/get', this.id)
    },
    post() {
      delete this.dataAdd.id
      delete this.dataAdd.fileExt
      this.dataAdd.type = 'nota-intern'
      this.$store.dispatch('internalnotes/postDisposition', { id: this.document.id, form: this.dataAdd, document: this.document })
      this.dialogAdd = false
      this.dataAdd = {}
    },
    remove() {
      this.$store.dispatch('internalnotes/removeDisposition', { id: this.document.id, data: this.dataEdit })
    ...
    download(document) {
      ...
      this.$store.dispatch('internalnotes/doneDisposition', { id: this.document.id, form: item })
    }
  },
  mounted() {
    this.get()
