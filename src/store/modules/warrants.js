...
const mutations = {
  setWarrants(state, val) {
    state.collection = val
  },
  setWarrant(state, val) {
    state.document = val
  },
}
...
const actions = {
  async post({ commit, dispatch }, form) {
    ...
    if(form.file){
      var file = form.file
      var storageRef = await storage.ref('warrants/' + id + '.' + file.name.split('.').pop());
      ...
      task.on('state_changed', snapshot => {
        ...
        await task.snapshot.ref.getDownloadURL().then(async downloadURL => {
          ...
          await ref.doc(id).set(form).then(() => {
            ...
            dispatch('notifications/post', {
              // title: 'Update profil berhasil.',
              body: `Surat perintah berhasil ditambahkan.`,
            }, { root: true })
          }).catch(err => {
            dispatch('notifications/post', {
              title: `Surat perintah gagal ditambahkan.`,
              body: err,
              timeout: 60
            }, { root: true })
      ...
      task.on('state_changed', snapshot => {
        ...
        await task.snapshot.ref.getDownloadURL().then(async downloadURL => {
          ...
          await ref.doc(id).set(form, { merge: true }).then(() => {
            ...
            dispatch('notifications/post', {
              // title: 'Update profil berhasil.',
              body: `Surat perintah berhasil diupdate.`,
            }, { root: true })
          }).catch(err => {
            dispatch('notifications/post', {
              title: `Surat perintah gagal diupdate.`,
              body: err,
              timeout: 60
            }, { root: true })
  ...
  async remove({ commit, dispatch }, fax) {
    ...
    await ref.doc(fax.id).delete().then(function() {
      if(fax.fileURL) {
        var storageRef = storage.ref('warrants/' + fax.id + '.' + fax.fileExt)
        storageRef.delete()
      }
      dispatch('get')
      commit('setLoading', null, { root: true })
      dispatch('notifications/post', {
        // title: 'Update profil berhasil.',
        body: `Surat perintah berhasil dihapus.`,
      }, { root: true })
    }).catch(err => {
      commit('setLoading', null, { root: true })
      dispatch('notifications/post', {
